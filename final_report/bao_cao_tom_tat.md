...
Nghiên cứu ứng dụng học máy vào dự báo rủi ro vỡ nợ tín dụng, sử dụng bộ dữ liệu *Give Me Some Credit* (Kaggle, 150.000 hồ sơ vay). Bốn mô hình được xây dựng theo thứ tự tăng dần độ phức tạp: Logistic Regression (AUC=0,8432), Decision Tree (AUC=0,8579), Random Forest (AUC=0,8703), XGBoost (AUC=0,8714). Feature engineering có chủ đích từ domain tài chính tạo ra hai đặc trưng xếp top-3 SHAP. Ngưỡng F2-tối ưu t=0,625 tăng Recall từ 51,6% lên 66,9%, giảm chi phí ước tính $2M so với ngưỡng F1-tối ưu (t=0,775). Sản phẩm là ứng dụng Streamlit giải thích từng quyết định tín dụng bằng SHAP waterfall.
...
| Feature engineering có cơ sở     | ≥ 2 features mới vào top-5 SHAP | **2/3** ✓               |
...
**Mất cân bằng lớp 1:14** → Accuracy vô nghĩa (model "đoán tất cả 0" đạt 93,3% nhưng Recall=0). Metrics phù hợp: AUC-ROC, F-beta, AUC-PR.
...
**Decision Tree CART** — phân chia đệ quy bằng Gini impurity: $G(t) = 2p_t(1-p_t)$. Kiểm soát overfitting bằng `max_depth` và `min_samples_leaf`.

**Random Forest** — bagging $B$ cây với random feature subsampling ($\sqrt{p}$ features/split). Variance giảm theo: $\text{Var} \to \rho\sigma^2$ khi $B\to\infty$, trong đó $\rho$ là tương quan cặp cây.
...
Leaf weight tối ưu dạng closed-form: $w_j^* = -G_j/(H_j+\lambda)$.

**SHAP** — Shapley value đo đóng góp cận biên của feature $j$:
...
**Give Me Some Credit** (Kaggle, 2011) — 149.999 hồ sơ vay tiêu dùng Mỹ, 10 features tài chính, target `SeriousDlqin2yrs` (trễ hạn ≥ 90 ngày trong 2 năm tiếp theo).
...
*Hình 3.1: Mất cân bằng lớp 1:14 buộc phải dùng AUC-ROC và F-beta thay vì Accuracy — model "đoán tất cả 0" đạt Accuracy=93,3% nhưng hoàn toàn vô dụng (Recall=0).*
...
Chi-squared test bác bỏ giả thuyết `MonthlyIncome` missing độc lập với target ($\chi^2=67,89, p\approx0$) → dữ liệu **MAR/MNAR**. Median imputation sẽ overestimate thu nhập của nhóm missing.
...
Bốn đặc trưng mới được tạo dựa trên domain tài chính:

| Feature mới | Công thức | Cơ sở |
|-------------|-----------|-------|
...
**Kết quả:** FinancialStressIndex (Spearman ρ=0,346) và TotalDelinquencyScore (ρ=0,345) dẫn đầu tương quan với target — vượt tất cả raw features gốc.
...
**Lưu ý:** Chỉ áp dụng trên training set — áp dụng trước split là data leakage.
...
**Stratified 70/15/15:** Training 104.999, Validation 22.500, Test 22.500 — mỗi tập giữ tỷ lệ 6,68%.
...
- **Tuning:** RandomizedSearchCV tối ưu AUC-ROC
- **Pipeline:** sklearn.Pipeline bọc preprocessing để tránh data leakage
...
- L1 zero-out `NumberOfTimes90DaysLate` → xác nhận `TotalDelinquencyScore` đã tổng hợp thông tin của nó.
...
- Node gốc phân chia tại TotalDelinquencyScore ≤ 2,5 — consistent với SHAP.
...
| Mô hình | AUC-ROC | F1 | Recall | Train time | Model size |
|---------|---------|-----|--------|------------|------------|
...
*Hình 4.3: 2/3 top features là do feature engineering tạo ra — FinancialStressIndex interaction term vượt cả raw components, xác nhận inductive bias từ domain tài chính có giá trị với mô hình phi tuyến mạnh.*

**SHAP base value:** 0,0148 (log-odds), tương ứng xác suất cơ sở 50,37% — hệ quả của `scale_pos_weight` điều chỉnh Bayesian prior.

## 4.5 Error Analysis

**Cấu trúc lỗi tại threshold=0,77:**

| Category | Số lượng | % |
|----------|---------|---|
| **TP** (bắt đúng defaults) | 776 | 51,6% actual defaults |
| **FN** (bỏ sót defaults) | 728 | **48,4% actual defaults** |
| **FP** (từ chối nhầm) | 1.195 | 5,7% non-defaults |
| **TN** (duyệt đúng) | 19.801 | 94,3% non-defaults |

**Profile FN (người vỡ nợ bị bỏ sót):** TotalDelinquencyScore median=0, MonthlyIncome=$4.200 (cao hơn TP). Đây là người vỡ nợ không có dấu hiệu cảnh báo trong lịch sử — giới hạn cấu trúc của features backward-looking.
...
*Hình 4.4: FN tập trung ở vùng score thấp (0,2–0,5) — người vỡ nợ "trông lành mạnh", phản ánh giới hạn căn bản của features lịch sử, không phải lỗi model.*

## 4.6 Threshold Optimization

Tìm threshold tối ưu F2 trên validation set:
$$t^* = \arg\max_{t} F_2(t) = 0,625$$

**Kết quả trên test set:**

| Threshold | F2 | Recall | Est. cost |
|-----------|----|----|-----------|
...
Gap cuối 0,021 < 0,03 → **"Good fit"**. Val AUC không còn tăng đáng kể sau N≈45.000 — giới hạn là chất lượng features, không phải số lượng dữ liệu.

## 4.8 Ba Tranh luận Lớn

**1. Interpretability vs Performance:** Logistic Regression dễ giải thích nhất nhưng AUC thấp hơn 2,8 điểm. Lựa chọn XGBoost bù đắp phần explainability bằng SHAP — thay vì nói "model từ chối", nhân viên tín dụng có thể trình bày: "TotalDelinquencyScore=12 đóng góp +0,45 vào xác suất vỡ nợ." Mức này đủ để giải thích quyết định cho khách hàng và phù hợp với yêu cầu trong quản trị rủi ro tín dụng.

**2. Data-Centric vs Model-Centric:** XGBoost với 10 features gốc → AUC≈0,855; thêm 4 engineered features → AUC=0,8714. Phần cải thiện không lớn về AUC, nhưng 2 trong 3 top-SHAP là features tự tạo — inductive bias từ domain tài chính vẫn có tác dụng ngay với model phi tuyến mạnh.

**3. Accuracy vs F1/AUC:** "Đoán tất cả 0" đạt Accuracy 93,3% nhưng Recall=0. AUC-ROC và F-beta là metrics phù hợp hơn với dữ liệu mất cân bằng. Ngưỡng mặc định 0,5 không tối ưu: cả 4 mô hình đều cần threshold > 0,5 (từ 0,62 đến 0,80) do class_weight dịch chuyển prior của model.
...
**Input:** 10 raw features → Tự tính 4 engineered features → DataFrame 14 features → XGBoost `predict_proba` → SHAP TreeExplainer → Output.

**Caching:** `@st.cache_resource` cho model và explainer (load một lần). SHAP waterfall tạo mới mỗi prediction.
...
Ngưỡng 62,5% aligned với t=0,625 từ F2-optimization.
...
- **Feature engineering validated:** FinancialStressIndex (#1 SHAP=0,577), TotalDelinquencyScore (#3 SHAP=0,410) — 2/3 top features
- **Threshold F2-optimal t=0,625:** Recall=66,9% (+15,3pp so với F1-opt), giảm chi phí $2M so với ngưỡng F1 (t=0,775)
- **Sản phẩm:** Streamlit dashboard SHAP-explainable, chạy local và deploy được

## 6.2 Hạn chế chính

1. **Features backward-looking:** 48,4% FN là người vỡ nợ không có dấu hiệu cảnh báo trong lịch sử — hạn chế cấu trúc của dữ liệu tín dụng lịch sử
2. **Dữ liệu tĩnh:** Không có time series → chỉ snapshot, không nắm trend
3. **Giới hạn địa lý:** Thị trường Mỹ — cần re-training và điều chỉnh ngưỡng cho Việt Nam
