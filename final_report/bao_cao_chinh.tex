...
với ngưỡng tối ưu theo $F_1$. Cuối cùng, toàn bộ pipeline được đóng gói thành một ứng
...
StandardScaler & Phép chuẩn hóa đưa mỗi đặc trưng về trung bình 0, độ lệch chuẩn 1.\\
...
hạn tập trung gần như toàn bộ ở 0. Hệ quả thực tiễn: các mô hình tuyến tính nhạy với cực
trị sẽ chịu thiệt, trong khi mô hình cây (chia theo ngưỡng, bất biến với biến đổi đơn
điệu) ít bị ảnh hưởng — một lý do nữa để kỳ vọng họ mô hình cây làm tốt hơn trên dữ liệu
này.
...
\Cref{fig:corr} là ma trận tương quan Pearson giữa các đặc trưng. Đáng chú ý nhất là khối
tương quan cao giữa ba biến trễ hạn (30--59, 60--89, 90+ ngày, tương quan đôi một
$0{,}98$--$0{,}99$): chúng mang thông tin trùng lặp nhiều. Quan sát này dẫn thẳng đến
quyết định ở \cref{sec:fe} — thay vì để mô hình tự xoay xở với ba biến gần như song song,
ta tổng hợp chúng thành một chỉ số trễ hạn duy nhất có trọng số. Ngoài khối này, các đặc
trưng còn lại tương quan yếu với nhau, tức mỗi biến đóng góp thông tin tương đối độc lập.
...
\Cref{fig:delinq} đào sâu nhóm biến trễ hạn theo nhãn: tỷ lệ vỡ nợ tăng vọt theo số lần
trễ hạn — hồ sơ chưa từng trễ hạn 90+ ngày vỡ nợ khoảng $4\%$, trong khi chỉ cần 1--2 lần
trễ 90+ ngày tỷ lệ này đã nhân lên nhiều lần. Đây là tín hiệu dự báo mạnh nhất trong dữ
liệu, và là cơ sở thực nghiệm cho cách đánh trọng số $3{:}2{:}1$ trong đặc trưng
\texttt{TotalDelinquencyScore} ở \cref{sec:fe}: mức trễ hạn càng nặng, liên hệ với vỡ nợ
càng dốc.
...
\end{itemize}
\Cref{fig:imputation} kiểm chứng lựa chọn này bằng trực quan: phân phối thu nhập sau khi
điền bằng KNN (đường liền) bám sát phân phối của phần dữ liệu quan sát được, trong khi
điền trung vị tạo một đỉnh nhọn nhân tạo tại giá trị điền. Bộ điền khuyết được khớp
(fit) \emph{chỉ trên tập huấn luyện} rồi áp sang tập kiểm định/kiểm tra, tránh rò rỉ
thông tin từ dữ liệu đánh giá.
...
\section{Chuẩn hóa}
Áp dụng \texttt{StandardScaler} (đưa mỗi đặc trưng về trung bình 0, độ lệch chuẩn 1)
\emph{chỉ cho Hồi quy Logistic}, vì hai lý do: thuật toán tối ưu hội tụ ổn định hơn khi
các đặc trưng cùng thang đo (thu nhập cỡ hàng nghìn USD đứng cạnh tỷ lệ cỡ $[0,1]$ làm
mặt mất mát rất dẹt theo một chiều), và hệ số sau chuẩn hóa so sánh được với nhau về độ
lớn. Các mô hình cây (DT, RF, XGBoost) chia nút theo \emph{thứ tự} giá trị nên bất biến
với mọi phép biến đổi đơn điệu — chuẩn hóa không thay đổi kết quả, do đó không áp dụng
để giữ nguyên thang đo gốc dễ diễn giải. Tương tự bộ điền khuyết, scaler chỉ được khớp
trên tập huấn luyện.
...
\begin{enumerate}
  \item \texttt{TotalDelinquencyScore} $=3\times(90{+})+2\times(60\text{--}89)+1\times(30\text{--}59)$.\\
        ...
        \emph{Học máy:} ba biến gốc tương quan đôi một $0{,}98$--$0{,}99$
        (\cref{fig:corr}) — gộp thành một trục duy nhất vừa khử trùng lặp, vừa cho mô
        hình tuyến tính một biến tổng hợp mà bản thân nó không thể tự tạo.
...
sánh thiếu công bằng. Siêu tham số được chọn bằng kiểm định chéo phân tầng $k=5$ trên tập
huấn luyện; chọn $k=5$ là điểm cân bằng thông dụng giữa độ ổn định của ước lượng và chi
phí tính toán~\cite[Ch.~5]{james2021}. Bốn mô hình được so sánh đúng theo trật tự lý
thuyết ở \cref{ch:lythuyet}: mốc tuyến tính $\to$ phi tuyến đơn $\to$ tổ hợp giảm phương
sai $\to$ tổ hợp giảm độ chệch.

\section{So sánh mô hình}
Kết quả tổng hợp cho trong \cref{tab:sosanh}. AUC tăng dần đúng theo trật tự độ phức tạp
của mô hình: LR ($0{,}8432$) $<$ DT ($0{,}8579$) $<$ RF ($0{,}8703$) $<$ XGBoost
($0{,}8714$), và mức tăng \emph{lý giải được bằng lý thuyết} ở \cref{ch:lythuyet}: bước
nhảy lớn nhất nằm giữa mô hình tuyến tính và họ mô hình cây (dữ liệu chứa nhiều quan hệ
phi tuyến và cực trị mà LR không biểu diễn được), tiếp đó các mô hình tổ hợp vượt cây đơn
nhờ giảm phương sai (RF) và giảm độ chệch tuần tự (XGBoost). Khoảng cách RF--XGBoost chỉ
$0{,}0011$ AUC — nhỏ, và kiểm định DeLong ở cuối chương xác nhận không có ý nghĩa thống
kê mạnh; XGBoost được chọn làm mô hình cuối vì đồng thời tốt hơn ở $F_1$/Recall, huấn
luyện nhanh hơn RF ($2{,}18$ so với $5{,}32$ giây) và hỗ trợ \texttt{scale\_pos\_weight}
tự nhiên trong hàm mất mát.
...
\Cref{fig:compare} trực quan hóa \cref{tab:sosanh} dưới dạng cột để so sánh đồng thời
nhiều chỉ số: dễ thấy bốn mô hình chênh nhau ít ở AUC nhưng phân hóa rõ hơn ở
Recall — đúng chỉ số quan trọng với tín dụng. \Cref{fig:roc} vẽ bốn đường ROC trên cùng
hệ trục: đường của RF và XGBoost gần như trùng nhau và cùng nằm trên LR/DT ở mọi mức
ngưỡng, nghĩa là ưu thế của mô hình tổ hợp không phụ thuộc vào việc chọn một ngưỡng cụ
thể nào.
...
(\cref{tab:shap}, \cref{fig:shapbar}), xác nhận hai ý đồ thiết kế ở \cref{sec:fe}: biến
tương tác ``dùng kiệt hạn mức kèm trễ hạn'' và chỉ số trễ hạn có trọng số thực sự mang
thông tin mà mô hình khai thác nhiều hơn cả các biến gốc tạo ra chúng.
...
trên toàn bộ tập kiểm tra — hiểu là ``trung bình mỗi dự báo, đặc trưng này đẩy xác suất
vỡ nợ đi bao xa''. Thứ tự cột vì vậy đọc được trực tiếp thành mức độ mô hình \emph{dựa
vào} từng đặc trưng: tỷ lệ dùng hạn mức đứng đầu (phù hợp trực giác nghiệp vụ — đây là
tín hiệu căng thẳng thanh khoản sớm nhất), theo sát là hai đặc trưng tự thiết kế.
...
$t^\star=\frac{500}{500+11.250}\approx 0{,}043$ — nhưng áp ngưỡng này đồng nghĩa từ chối
$\sim 74\%$ hồ sơ, bất khả thi về kinh doanh (ngân hàng không thể từ chối ba phần tư
khách hàng). Nguyên nhân là quy tắc Bayes chỉ tối ưu chi phí \emph{trong mô hình giả
định}, bỏ qua các ràng buộc vận hành và sai số hiệu chỉnh xác suất. Vì vậy đồ án dùng nó
làm mốc tham chiếu và chọn ngưỡng triển khai bằng cách tối ưu trực tiếp $F_2$
(\cref{eq:fbeta}, $\beta=2$ — ưu tiên Recall gấp đôi, nhất quán với hướng bất đối xứng
chi phí nhưng không cực đoan): quét $t\in[0{,}05;0{,}95]$ trên tập kiểm định cho cực đại
tại $t=0{,}625$ với Recall $66{,}9\%$, Precision $29{,}9\%$, $F_2=0{,}537$
(\cref{tab:nguong}, \cref{fig:threshold}).
...
\Cref{fig:threshold} thể hiện quá trình quét ngưỡng: trục hoành là ngưỡng $t$, các đường
cong là Recall (giảm dần theo $t$), Precision (tăng dần) và $F_2$; điểm cực đại của đường
$F_2$ chính là ngưỡng $0{,}625$ được chọn. \Cref{tab:nguong} đặt bốn ngưỡng cạnh nhau để
thấy rõ đánh đổi: ngưỡng Bayes lý thuyết cho Recall gần tuyệt đối nhưng chi phí và tỷ lệ
từ chối phi thực tế; ngưỡng $F_1$ cân Precision--Recall đều nhau nên bỏ sót gần một nửa
hồ sơ vỡ nợ; ngưỡng $F_2$ được chọn vì giữ Recall ở mức cao ($66{,}9\%$) trong khi tỷ lệ
từ chối còn chấp nhận được — so với ngưỡng $F_1$, nó giảm khoảng $2{,}01$ triệu USD chi
phí ước tính trên tập kiểm tra.
...
\Cref{fig:score} vẽ phân bố điểm rủi ro của hai lớp: lớp tốt dồn về điểm thấp, lớp vỡ nợ
trải về phía điểm cao, và phần \emph{giao nhau} của hai phân bố quanh ngưỡng chính là nơi
sinh ra hầu hết FP/FN — các hồ sơ mang tín hiệu mâu thuẫn (ví dụ thu nhập tốt nhưng đã
từng trễ hạn). Hàm ý vận hành: thay vì tin tưởng tuyệt đối quyết định nhị phân, vùng điểm
sát ngưỡng nên được chuyển sang thẩm định thủ công — mô hình lo phần dễ chiếm đa số, con
người lo phần khó.
...
Về chất lượng xác suất: sau Platt scaling, điểm Brier giảm từ $0{,}0582$ xuống $0{,}0478$
và ECE từ $3{,}2\%$ xuống $1{,}9\%$. \Cref{fig:calib} cho thấy điều này bằng hình: trục
hoành là xác suất mô hình dự báo, trục tung là tần suất vỡ nợ thực tế trong từng khoảng
điểm; đường chéo $45^\circ$ là hiệu chỉnh hoàn hảo. Đường của mô hình gốc võng khỏi đường
chéo (đặc trưng của boosting~\cite{niculescu2005}), còn đường sau Platt scaling bám sát
đường chéo hơn rõ rệt — tức con số ``xác suất vỡ nợ $30\%$'' mà ứng dụng hiển thị thực sự
gần nghĩa đen của nó. Cuối cùng, kiểm định DeLong cho cặp XGBoost--Random Forest trên
cùng tập kiểm tra xác nhận chênh lệch AUC $0{,}0011$ không có ý nghĩa thống kê mạnh —
nhất quán với nhận định ở \cref{tab:sosanh}, và củng cố rằng lý do chọn XGBoost nằm ở
Recall, tốc độ và cơ chế trọng số lớp chứ không phải ở một khác biệt AUC mong manh.
...
thuộc \emph{bất kỳ dạng nào} (kể cả phi tuyến) giữa từng đặc trưng và nhãn vỡ nợ, bổ sung
cho tương quan Pearson vốn chỉ bắt quan hệ tuyến tính. Thứ hạng cao của nhóm trễ hạn và
các đặc trưng tự thiết kế ở đây nhất quán với kết quả SHAP ở \cref{sec:shap_kq} — hai
phương pháp độc lập cùng chỉ về một nhóm tín hiệu.
...
\label{fig:lc}
\end{figure}
\noindent\textbf{Về \cref{fig:lc}:} đường cong học vẽ hiệu năng trên tập huấn luyện và
tập kiểm định theo số mẫu huấn luyện. Hai đường hội tụ về gần nhau khi đủ dữ liệu, không
có khoảng hở lớn — dấu hiệu mô hình không quá khớp nghiêm trọng và lượng dữ liệu hiện có
đã gần đủ ``no'' cho cấu hình mô hình này (thu thập thêm dữ liệu cùng loại sẽ không cải
thiện nhiều; muốn tốt hơn cần đặc trưng mới hoặc nguồn dữ liệu mới).
...
\label{fig:delong}
\end{figure}
\noindent\textbf{Về \cref{fig:delong}:} ma trận giá trị $p$ của kiểm định DeLong cho
từng cặp mô hình trên cùng tập kiểm tra. Các cặp liên quan đến LR/DT đều khác biệt có ý
nghĩa (giá trị $p$ rất nhỏ), riêng cặp RF--XGBoost không đạt ngưỡng ý nghĩa thống kê —
cơ sở định lượng cho phát biểu ở \cref{ch:thucnghiem} rằng việc chọn XGBoost dựa trên
Recall, tốc độ và cơ chế trọng số lớp chứ không dựa trên chênh lệch AUC.
