...
Báo cáo trình bày nghiên cứu ứng dụng học máy vào bài toán dự báo rủi ro vỡ nợ tín dụng, sử dụng bộ dữ liệu *Give Me Some Credit* (Kaggle, 150.000 hồ sơ vay). Bốn mô hình học có giám sát được xây dựng và đánh giá theo thứ tự tăng dần độ phức tạp: Logistic Regression (AUC=0,8432), Decision Tree CART (AUC=0,8579), Random Forest (AUC=0,8703), và XGBoost (AUC=0,8714). Mô hình tốt nhất — XGBoost với cơ chế boosting bậc hai — vượt mục tiêu đặt ra (AUC > 0,87) và được phân tích sâu qua SHAP TreeExplainer. Trích xuất đặc trưng có chủ đích (TotalDelinquencyScore, FinancialStressIndex) được SHAP xác nhận là 2/3 đặc trưng quan trọng nhất. Ngưỡng F2-tối ưu t=0,625 tăng Recall từ 51,6% lên 66,9% và giảm ước tính chi phí $2 triệu so với ngưỡng F1-tối ưu (t=0,775), là điểm thỏa hiệp giữa tối thiểu hóa FN và kiểm soát FP. Sản phẩm cuối là ứng dụng Streamlit tương tác, giải thích từng quyết định tín dụng bằng SHAP waterfall chart.
...
Sự phát triển của học máy mở ra khả năng xây dựng mô hình chấm điểm tín dụng tự động, học trực tiếp từ lịch sử hàng triệu hồ sơ, nắm bắt được cả những tương tác phi tuyến phức tạp. Đồng thời, kỹ thuật SHAP (SHapley Additive exPlanations) giải quyết vấn đề "hộp đen" — yêu cầu pháp lý bắt buộc ngân hàng phải giải thích lý do từ chối cho khách hàng (Basel III, Điều 431).
...
| Trích xuất đặc trưng có cơ sở | ≥ 2 đặc trưng mới vào top-5 SHAP | **2/3** trong top SHAP |
...
**Hệ quả 1 — Độ chính xác (Accuracy) không phù hợp và không đáp ứng được yêu cầu của bài toán thực tế:** Mô hình tầm thường $f(\mathbf{x}) \equiv 0$ (đoán tất cả không vỡ nợ) đạt Accuracy = 93,32% nhưng Recall = 0 — không có giá trị ứng dụng trong phát hiện rủi ro tín dụng.
...
**Ý nghĩa tài chính:** $e^{\beta_j}$ là *odds ratio* của đặc trưng $j$ — tăng $x_j$ lên 1 đơn vị nhân odds vỡ nợ lên $e^{\beta_j}$ lần.
...
**Giải thích trực quan:** Tạp chất Gini đo mức độ hỗn tạp của một node, tức là xác suất một mẫu ngẫu nhiên bị phân loại sai nếu gán nhãn ngẫu nhiên theo phân phối trong node đó. Node thuần khiết có $G = 0$ vì toàn bộ mẫu cùng lớp — không có sự không chắc chắn; node hỗn tạp tối đa ($p_t = 0,5$) có $G = 0,5$ vì xác suất phân loại sai cao nhất.
...
### 2.3.2 Kiểm soát Quá khớp qua Cắt tỉa

Cây đầy đủ chiều sâu quá khớp dữ liệu huấn luyện. Kiểm soát qua:
...
### 2.4.3 Tầm quan trọng Đặc trưng
...
### 2.5.3 Trọng số Lá tối ưu dạng kín
...
*Hình 3.1: Mất cân bằng dữ liệu 1:14 — mô hình "đoán tất cả 0" đạt Accuracy=93,3% nhưng Recall=0, cho thấy Độ chính xác (Accuracy) không phù hợp và không đáp ứng yêu cầu bài toán thực tế, bắt buộc phải dùng AUC-ROC và F-beta.*
...
Pearson correlation RevolvingUtil — target = −0,002 (gần zero), trong khi Spearman ρ = +0,24 (tương quan đáng kể). Điều này chứng minh tồn tại quan hệ **phi tuyến đơn điệu** (đơn điệu phi tuyến): tỷ lệ vỡ nợ tăng theo hàm bậc thang, không theo đường thẳng. Các mô hình dựa trên cây nắm bắt được điều này; Logistic Regression không.

### 3.2.2 Hệ số tương quan giữa các biến delinquency

Spearman ρ(90+days, 60-89days) = 0,49, ρ(90+days, 30-59days) = 0,45. Đa cộng tuyến cao → VIF = ∞ cho các cặp này. Giải pháp: tổng hợp vào `TotalDelinquencyScore` thay vì dùng riêng lẻ.

![Ma trận tương quan Spearman giữa các đặc trưng](../reports/fig_05_correlation_heatmap.png)
*Hình 3.2b: Ma trận tương quan Spearman — cho thấy đa cộng tuyến cao giữa ba biến delinquency (hệ số 0,45–0,49), là cơ sở kỹ thuật để nén chúng thành `TotalDelinquencyScore`. `DebtRatio` và `MonthlyIncome` gần như không tương quan (ρ ≈ 0), cho thấy thông tin bổ sung từ cả hai biến này.*
...
**KNN Imputation** (k=5, nan_euclidean distance): ước lượng $x_{i,\text{income}}$ bằng trung bình có trọng số của 5 láng giềng gần nhất trong không gian đặc trưng còn lại:
...
**Spearman ρ với biến mục tiêu:** 0,345 — cao hơn bất kỳ đặc trưng trễ hạn thô nào riêng lẻ (max 0,342 với `NumberOfTimes90DaysLate`). Nén thành một điểm số giảm đa cộng tuyến đồng thời tăng sức mạnh dự báo.
...
*Hình 3.3: FinancialStressIndex (ρ=0,346) và TotalDelinquencyScore (ρ=0,345) dẫn đầu tương quan với target, vượt các đặc trưng gốc thô — xác nhận trích xuất đặc trưng có chủ đích theo lĩnh vực tài chính mang lại giá trị dự báo cao hơn.*
...
**class_weight='balanced':** Gán trọng số $w_+ = N/(2N_+) = 14,0$ cho lớp thiểu số trong hàm loss. Không tạo dữ liệu tổng hợp, không ảnh hưởng phân phối đặc trưng.
...
*Hình 3.4: SMOTE cân bằng hoàn toàn tập huấn luyện (50/50) bằng cách tạo mẫu tổng hợp nội suy giữa các điểm lớp thiểu số — nhưng không được áp dụng trên val/test để tránh rò rỉ dữ liệu.*
...
**Điều chỉnh siêu tham số:** `RandomizedSearchCV` — ưu tiên hơn `GridSearchCV` khi không gian tìm kiếm lớn (thời gian tuyến tính theo số iterations thay vì exponential theo số tham số). Số iterations: LR=12, DT=20, RF=15, XGB=15.

**Metric tối ưu trong CV:** AUC-ROC (không phụ thuộc ngưỡng, phù hợp so sánh mô hình).

**Quy trình xử lý:** Tất cả preprocessing (imputation, scaling) được đóng gói trong `sklearn.Pipeline` để tránh rò rỉ dữ liệu giữa train và val folds.
...
Đáng chú ý là L1 (C=0,001) đưa hệ số `NumberOfTimes90DaysLate` về 0, trong khi `TotalDelinquencyScore` — vốn đã mã hóa thông tin của đặc trưng đó — vẫn được giữ lại với hệ số dương. Đây là bằng chứng gián tiếp cho thấy trích xuất đặc trưng theo hướng đúng.
...
Top 5: TotalDelinquencyScore, FinancialStressIndex, RevolvingUtilization, age, MonthlyIncome — nhất quán với tầm quan trọng toàn cục SHAP của XGBoost. Các đặc trưng được tạo lại xếp đầu.
...
| learning_rate | 0,05 | Co rút — học chậm, tổng quát hóa tốt hơn |
...
*Hình 4.2: FinancialStressIndex (mean|SHAP|=0,577) và TotalDelinquencyScore (0,410) — 2 đặc trưng được tạo thủ công — chiếm vị trí #1 và #3, xác nhận thiên kiến quy nạp từ lĩnh vực tài chính có giá trị ngay cả với mô hình phi tuyến mạnh.*

**Phát hiện đáng chú ý:**
- `FinancialStressIndex` (#1, 0,577) > `RevolvingUtilization` (#2, 0,535): tương tác phi tuyến vượt thành phần gốc — nắm bắt mối tương quan giữa tỷ lệ sử dụng tín dụng cao và lịch sử trễ hạn
...
3. XGBoost vượt RF về: thời gian huấn luyện (127s vs 511s), kích thước mô hình (340KB vs 12MB), thời gian suy diễn (0,79 µs vs 4,82 µs/record — 6× nhanh hơn), khả năng giải thích (SHAP chính xác)
4. **Mô hình tốt nhất: XGBoost** → lưu `models/best_model.pkl`

### 4.6.1 Kiểm định Thống kê Sự khác biệt AUC — DeLong Test

AUC point estimate giữa các mô hình có khoảng cách nhỏ (RF ≈ XGB) nhưng để khẳng định "XGBoost vượt trội" cần kiểm định thống kê có nguyên tắc. Kiểm định DeLong [13] so sánh hai AUC-ROC từ cùng tập kiểm tra, có tính đến tương quan giữa hai bộ dự báo (cùng dữ liệu nên hai estimator không độc lập — bỏ qua hiệp phương sai sẽ khiến phương sai bị ước tính quá cao và mất năng lực kiểm định).

**Phương pháp — U-statistic của AUC:**

Ký hiệu $n_+ = 1.504$ (số positives trong tập kiểm tra), $n_- = 20.996$ (số negatives). AUC được ước lượng qua structural components:
...
| Ngưỡng | F1 | F2 | Precision | Recall | Chi phí ước tính |
|-----------|----|----|-----------|--------|-----------|
...
Gap cuối = 0,021 < 0,03 → **"Khớp tốt"**: không quá khớp nghiêm trọng. AUC tập kiểm định không còn tăng đáng kể sau N≈45.000 → thêm dữ liệu ít giá trị. Giới hạn nằm ở chất lượng đặc trưng, không phải số lượng dữ liệu.

![Learning curve XGBoost — train AUC và val AUC theo kích thước tập huấn luyện](../reports/fig_29_learning_curve.png)
*Hình 4.6: Gap train-val thu hẹp từ 0,117 (10% data) xuống 0,021 (100% data) — mô hình không quá khớp và đã hội tụ; val AUC không còn tăng đáng kể sau N≈45.000, giới hạn là chất lượng đặc trưng, không phải thiếu dữ liệu.*
...
- XGBoost với 10 đặc trưng gốc (không có trích xuất, tự học tương tác) → AUC ước tính ~0,855
- XGBoost với 14 đặc trưng (có trích xuất) → AUC=0,8714

Kết quả: Hướng tiếp cận tập trung vào dữ liệu (trích xuất đặc trưng có chủ đích) kết hợp hướng tiếp cận tập trung vào mô hình (XGBoost) cho kết quả tốt nhất. Trích xuất đặc trưng cung cấp thiên kiến quy nạp phù hợp lĩnh vực tài chính giúp mô hình hội tụ nhanh hơn và tốt hơn với cùng lượng dữ liệu.

**Bằng chứng từ SHAP:** 2/3 đặc trưng hàng đầu là đặc trưng được tạo — chứng minh trích xuất đặc trưng không thừa, ngay cả với mô hình phi tuyến mạnh như XGBoost.

### 4.8.3 Độ chính xác so với Recall — Chiến lược Ngưỡng

Độ chính xác (Accuracy) 93,3% của mô hình "đoán tất cả 0" cho thấy Accuracy không phù hợp để đánh giá với dữ liệu mất cân bằng. Thay vào đó:
...
**10 đặc trưng gốc** người dùng nhập thủ công trong form (theo đúng thứ tự trong `app.py`):

| # | Tên đặc trưng | Ý nghĩa tiếng Việt | Phạm vi | Kiểu |
|---|--------------|-------------------|---------|------|
| 1 | `RevolvingUtilizationOfUnsecuredLines` | Tỷ lệ sử dụng hạn mức tín dụng | 0,0 – 1,0 | Thực |
...
| `DebtToIncomeRatio` | $\text{DebtRatio} \times \text{MonthlyIncome}$ | Tổng nợ tuyệt đối (USD/tháng) |
...
- **Minh bạch quyết định bằng SHAP:** SHAP TreeExplainer cung cấp giải thích tại mức từng hồ sơ (giải thích cục bộ) và toàn bộ bộ dữ liệu (giải thích tổng quát), đáp ứng yêu cầu explainability theo Basel III Pillar 3 và GDPR Article 22.
...
**4. Hiệu chỉnh xác suất:** Mô hình được tối ưu cho khả năng phân biệt (AUC) nhưng chưa hiệu chỉnh để đầu ra xác suất có ý nghĩa tuyệt đối (tức P(default)=0,7 thực sự nghĩa là 70%). Hiệu chỉnh Platt hoặc hồi quy isotonic có thể cải thiện.
...
- **Dữ liệu thay thế:** Dữ liệu hành vi (thanh toán di động, hóa đơn tiện ích) → tăng coverage cho nhóm khách hàng ít lịch sử tín dụng.
...
- **Mạng nơ-ron đồ thị:** Mô hình hóa mối quan hệ giữa người vay (mạng lưới bảo lãnh) → rủi ro xã hội.
- **Suy diễn nhân quả:** Phân biệt tương quan và quan hệ nhân quả trong các yếu tố tín dụng — quan trọng trong xây dựng chính sách.
- **Học liên kết (Federated learning):** Huấn luyện mô hình trên dữ liệu phân tán giữa nhiều tổ chức tín dụng mà không chia sẻ dữ liệu thô (tuân thủ GDPR).
...
│   └── features.py             ← Hàm trích xuất đặc trưng
└── final_report/
